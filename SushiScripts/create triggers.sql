...
BEGIN
	...
	SET CT.DONGIATONG = 
		CASE
			WHEN M.MAMON IS NOT NULL THEN CT.SOLUONG * M.GIAHIENTAI
			WHEN M.MACOMBO IS NOT NULL THEN CT.SOLUONG * M.GIAHIENTAI
			ELSE 0
		END
...
BEGIN
	...
	IF EXISTS (
		SELECT 1
		FROM INSERTED I
		JOIN MENU M ON I.MAMENU = M.MAMENU
		WHERE 
			(M.MAMON IS NOT NULL AND NOT EXISTS (SELECT 1 FROM MONAN MA WHERE MA.MAMON = M.MAMON))
			OR 
			(M.MACOMBO IS NOT NULL AND NOT EXISTS (SELECT 1 FROM COMBOMONAN CB WHERE CB.MACOMBO = M.MACOMBO))
	)
	BEGIN
...
CREATE TRIGGER TRG_MENU_MAMON_MACOMBO
ON MENU
FOR INSERT, UPDATE
AS
BEGIN
...
BEGIN
	IF NOT EXISTS(
		...
		JOIN MENU M ON M.MAMENU = C.MAMENU
		JOIN MONAN MA ON M.MAMON = MA.MAMON
